...
class quad_control:
    def __init__(self):
        ...
        self.x_pid = pid(0.6, 0.0, 0.1, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        self.y_pid = pid(0.6, 0.0, 0.1, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        self.z_pid = pid(0.8, 0.0, 0.2, filter_tau, dt, minVal = minVel, maxVal=maxVel)


        ########################## TODO - SET GAINS HERE ##############################
        ###############################################################################
        # NED velocity controller. EDIT GAINS HERE
        self.vx_pid = pid(0.6, 0.1, 0.2, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        self.vy_pid = pid(0.6, 0.1, 0.2, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        self.vz_pid = pid(0.8, 0.2, 0.3, filter_tau, dt, minVal = minAcc, maxVal = maxAcc)
        ##############################################################################
        ##############################################################################
