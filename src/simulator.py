...
class LiveQuadrotorSimulator:
    ...
    def animated_rrt_planning(self, start=None, goal=None):
        ...
        max_iterations = max(4500, max(1000, int(distance * 150)))  # Scale with distance
        step_size = min(1.5, distance / 10)  # Adaptive step size
        goal_radius = max(0.8, min(1.5, distance / 15))  # Adaptive goal radius
        search_radius = step_size * 4.6
        # goal_bias = 0.15
        goal_bias = 0.11
        # RRT* parameters - adjust based on environment size
        # max_iterations = max(14500, max(1000, int(distance * 150)))  # Scale with distance
        # step_size = min(0.5, distance / 10)  # Adaptive step size
        # goal_radius = min(0.8, min(1.5, distance / 15))  # Adaptive goal radius
        # search_radius = step_size * 3.6
        # # goal_bias = 0.15
        # goal_bias = 0.08
